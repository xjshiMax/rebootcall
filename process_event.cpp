...
int FScall::LauchFScall()
{
	...
	{
		sprintf(callCmd,"bgapi originate {speechCraftID=%s,taskID=%s,taskname=%s,username=%s}sofia/gateway/%s/83%s &park()",m_speechcraftID.c_str(),m_taskID.c_str(),m_taskName.c_str(),(ite->username).c_str(),m_gateway.c_str(),(ite->phonenum).c_str());
